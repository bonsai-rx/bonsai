...
namespace Bonsai.IO
{
    ...
    [Description("Returns an array with the names of files and subdirectories that match the specified search pattern.")]
    public class GetFileSystemEntries : Source<string[]>
    {
        ...
        /// (see <see cref="Directory.GetFileSystemEntries(string, string, SearchOption)"/>).
        /// </summary>
        ...
        {
            return Observable.Defer(() => Observable.Return(Directory.GetFileSystemEntries(Path, SearchPattern, SearchOption)));
        }
    }
}
