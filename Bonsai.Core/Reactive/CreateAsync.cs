...
namespace Bonsai.Reactive
{
    ...
    public class CreateAsync : WorkflowExpressionBuilder
    {
        static readonly Expression UnitExpression = Expression.Constant(Observable.Return(Unit.Default), typeof(IObservable<Unit>));
        static readonly Range<int> argumentRange = Bonsai.Range.Create(lowerBound: 0, upperBound: 1);

        /// <summary>
        /// Initializes a new instance of the <see cref="CreateAsync"/> class.
        /// </summary>
        public CreateAsync()
        ...
        /// </param>
        public CreateAsync(ExpressionBuilderGraph workflow)
            : base(workflow)
        {
        }
        ...
        public override Expression Build(IEnumerable<Expression> arguments)
        {
            ...
            return BuildWorkflow(arguments.Select(x => UnitExpression), null, selectorBody =>
            {
                ...
                if (source != null)
                {
                    ...
                    return Expression.Call(typeof(CreateAsync), nameof(Process), new[] { sourceType, resultType }, source, factory);
                }
                else return Expression.Call(typeof(CreateAsync), nameof(Process), new[] { resultType }, factory);
            });
        }
